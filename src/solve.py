...
def solve_no_limit_best_11(input_folder, output_folder):
    ...
    result_df = result_df[['player_id', 'web_name', 'team_code', 'element_type', 'now_cost', 'event', 'points_md', 'is_captain', 'multiplier']].copy()
...
def solve_limited_best_squad(input_folder, output_folder):
    ...
    result_df = pd.merge(next_week_df, results, on='player_id', how='inner')
    result_df = result_df[['player_id', 'web_name', 'team_code', 'element_type', 'now_cost', 'event', 'points_md', 'is_captain', 'multiplier', 'starting_lineup']].copy()
    result_df['gw_points'] = result_df['multiplier'] * result_df['points_md']
