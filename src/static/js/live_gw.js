...
var app = new Vue({
  ...
  computed: {
    valid_team_id() {
      return this.team_id == -1 || this.team_id == -2
        ? "Click to enter"
        : this.show_team_info
        ? this.team_id
        : "Hidden";
    ...
    is_all_ready() {
      return (
        this.is_xp_ready &&
        this.is_fixture_ready &&
        this.is_el_ready &&
        this.is_ownership_ready
      );
    },
    is_ready() {
      return this.team_id == -1 ||
        this.team_data == undefined ||
        this.team_data.length == 0
        ? false
        : true;
    },
    is_team_info_ready() {
      return this.team_info !== undefined && !_.isEmpty(this.team_info);
    },
    is_rp_ready() {
      return this.rp_data !== undefined && this.rp_data.length != 0;
    },
    is_xp_ready() {
      return this.xp_data !== undefined && this.xp_data.length != 0;
    },
    is_fixture_ready() {
      return this.gw_fixture !== undefined && this.gw_fixture.length != 0;
    },
    is_el_ready() {
      return this.el_data !== undefined && this.el_data.length != 0;
    },
    is_ownership_ready() {
      return (
        this.ownership_data !== undefined && this.ownership_data.length != 0
      );
    },
    seasongwdate: {
      get: function () {
    ...
    ownership_data() {
      if (!this.is_using_autosub || !this.is_using_sample) {
        let own_data = get_ownership_by_type(
          this.ownership_source,
          this.el_data,
          this.sample_data,
          {}
        );
        ...
        el_data.forEach((e) => {
          autosubs.push([
            e.id,
            {
              ...
              autosub:
                rp_by_id && rp_by_id[e.id] ? rp_by_id[e.id].autosub : false,
            },
          ]);
        });
    ...
    sorted_ownership() {
      if (_.isEmpty(this.element_data_combined)) {
        this.sorted_ownership_cache = [];
        return [];
      }
      const all_elements = Object.values(this.element_data_combined);
      let sorted_own = _.orderBy(
        all_elements,
        ["ownership", "xp"],
        ["desc", "desc"]
      );
    ...
    team_eo_defense() {
      ...
      data.forEach((game) => {
        let teams = [
          teams_ordered[game["team_h"] - 1],
          teams_ordered[game["team_a"] - 1],
        ];
    ...
    rp_by_id() {
      ...
      rp_original.forEach((p) => {
        let total_score = getSum(
          p.explain.map((i) => i.stats.map((j) => j.points)).flat()
        );
        p.rp = total_score;
      ...
      override.forEach((e) => {
        ...
        b.explain = [
          {
            fixture: b.explain[0].fixture,
            stats: [{ identifier: "override", points: e.rp, value: 0 }],
          },
        ];
        rp_obj[e.id] = b;
      });
    ...
    xp_storage_this_gw() {
      if (_.isEmpty(this.xp_storage)) {
        return false;
      }
      let this_gw = parseInt(this.gw.replace("GW", ""));
      return !_.isEmpty(
        this.xp_storage.find(
          (i) => i.meta.start_gw == this_gw && i.meta.status == "active"
        )
      );
    },
    gameweek_info() {
      if (this.gw_fixture.length == 0) {
        return {};
      }
    ...
    grouped_xp_data() {
      ...
      let pts_grouped = _(pts)
        .groupBy("player_id")
        .values()
        .map((group) => ({
          ...group[0],
          qty: group.length,
          event_list: group.map((k) => parseInt(k.event_id)),
        }))
        .value();
      return pts_grouped;
    },
    gameweek_games_with_metadata() {
      if (!this.is_fixture_ready) {
        return [];
      }
      if (!this.is_xp_ready) {
        return [];
      }
      if (!this.is_ownership_ready) {
        return [];
      }
      ...
      cloned_fixture.forEach((game) => {
        ...
        if (game.started && !game.finished) {
          try {
            ...
            let home_players = bps_stats.h.map((i) => {
              i["home"] = true;
              return i;
            });
            let away_players = bps_stats.a.map((i) => {
              i["home"] = false;
              return i;
            });
            let all_players = home_players.concat(away_players);
            let sorted_groups = Object.entries(
              _.groupBy(all_players, (i) => i.value)
            )
              .sort((a, b) => b[0] - a[0])
              .slice(0, 3);
            let count = 3;
            ...
            let final_list = sorted_groups
              .filter((i) => i[2])
              .map((i) => i[1])
              .flat();
            let prov_bps_stats = {
              identifier: "bps_provisional",
              a: final_list.filter((i) => !i.home),
              h: final_list.filter((i) => i.home),
            };
        ...
        player_with_data.forEach((e) => {
          ...
          if (rp_ready) {
            ...
            if (find == undefined) {
              ...
              e.rp_detail = rp_by_id[e.id].explain
                .find((i) => i.fixture == game.id)
                .stats.map((i) => [i.identifier, i.points, i.value]);
          ...
          e.ownership = this.is_using_sample
            ? ((ownership_vals[e.id] &&
                ownership_vals[e.id].effective_ownership) ||
                0) / 100
            : ((ownership_vals[e.id] &&
                ownership_vals[e.id].selected_by_percent) ||
                0) / 100;
        ...
        game.players_owned = getSum(
          game.player_details.map((i) => i.multiplier)
        );
        game.players_nonowned = nonowned_players.length;
        game.players_owned_eo = getSum(owned_players.map((i) => i.eo_owned));
        game.players_nonowned_eo = getSum(
          nonowned_players.map((i) => i.eo_nonowned)
        );
      ...
      cloned_fixture.forEach((game_info) => {
        ...
        game_info.player_details.forEach((p) => {
          p.web_name = xp_by_id[p.id].web_name;
        });
        game_info.player_details.sort((a, b) => {
          return b.rp - a.rp || b.xp - a.xp;
        });
        game_info.players = _.groupBy(game_info.player_details, (i) =>
          el_by_id[i.id].team == game_info.team_h ? "home" : "away"
        );
      });
    ...
    player_counts() {
      if (_.isEmpty(this.gameweek_games_with_metadata)) {
        return {};
      }

      let game_details = this.gameweek_games_with_metadata;
      let your_total = getSum(
        game_details
          .map((i) => i.player_details.map((i) => i.multiplier))
          .flat()
      );
      let your_played = getSum(
        game_details
          .filter((i) => i.started)
          .map((i) => i.player_details.map((i) => i.multiplier))
          .flat()
      );
      let avg_total = getSum(
        game_details.map((i) => i.player_details.map((i) => i.ownership)).flat()
      );
      let avg_played = getSum(
        game_details
          .filter((i) => i.started)
          .map((i) => i.player_details.map((i) => i.ownership))
          .flat()
      );
    ...
    get_graph_checkpoints() {
      ...
      let all_discrete_events = cloned_fixture
        .map((i) => [
          { type: "start", dt: i.start_dt, game: i },
          { type: "end", dt: i.end_dt, game: i },
        ])
        .flat();
      all_discrete_events.push({
        type: "now",
        dt: new Date(app.now_dt),
        game: undefined,
      });
      all_discrete_events.sort((a, b) => {
        return a.dt - b.dt;
      });
      ...
      if (this.is_using_hits) {
        initial_team = -(
          (this.team_data.entry_history &&
            this.team_data.entry_history.event_transfers_cost) ||
          0
        );
      }
      if (this.is_using_hits && this.is_using_sample) {
        let sample_d = this.current_sample_data;
        let vals = sample_d.map(
          (i) => i.data.entry_history.event_transfers_cost
        );
      ...
      all_discrete_events.forEach((event) => {
        ...
        if (event.type == "start") {
          ...
          target_event.active_events = target_event.active_events.filter(
            (i) => i.game.id !== event.game.id
          );
        ...
        let values = this.get_points_for_time(
          event.type,
          current_time,
          target_event.active_events,
          target_event.finished_events,
          setoff,
          now_values
        );
        ...
        let existing_id = team_checkpoints.findIndex(
          (i) => i.time == current_time
        );
        if (existing_id == -1) {
          let entry = {
    ...
    selected_game_info() {
      if (this.modal_selected_game == undefined) {
        return undefined;
      }
    ...
    element_data_combined() {
      ...
      const xp_by_id = Object.fromEntries(
        this.grouped_xp_data.map((i) => [i.player_id, i])
      );
      const el_by_id = this.el_by_id;
      if (_.isEmpty(el_by_id)) {
        return {};
      }
      const rp_by_id = this.rp_by_id;
      ...
      all_ids.forEach((e) => {
        try {
          ...
          n.autosub = false;
          if (n.rp_data !== undefined && n.rp_data.autosub) {
            n.autosub = true;
          }
          ...
          let multiplier = (n.multiplier = is_squad
            ? picks.find((i) => i.element == e).multiplier
            : 0);
          n.is_lineup = multiplier > 0;
          let ownership = (n.ownership = this.is_using_sample
            ? n.own_data.effective_ownership
            : parseFloat(n.own_data.selected_by_percent));
    ...
    team_data_with_metadata() {
      ...
      picks.forEach((player) => {
        ...
        let cnt = picks
          .filter((j) => j.element_type == data.element_type)
          .filter((j) => j.multiplier > 0).length;
        if (data.element_type == 5) {
          // AM
          player.x = (122 / (5 + has_am)) * 1 - 17;
          player.y = 138.5;
        } else if (data.multiplier > 0) {
          player.x =
            (122 / (cnt + 1)) * pos_ctr[parseInt(player.element_type)] - 17;
    ...
    potential_targets() {
      ...
      els = els.filter(
        (i) => i.element_type == this.target_player.element_type
      );
      els.sort((a, b) => {
        return b.xp - a.xp;
      });
    ...
    currentTeamFormation() {
      ...
      let picks = this.team_data_with_metadata
        .filter((i) => i.multiplier > 0)
        .map((i) => i.element_type);
    ...
    autosub_subs() {
      ...
      let top_ones = Object.entries(_.countBy(stats))
        .sort((a, b) => b[1] - a[1])
        .slice(0, 15);
      const top_pairs = top_ones.map((s) => {
        ...
        return {
          ...
          to: el[players[1]],
          change: change,
          effect: getWithSign((-s[1] / count) * change),
        };
      });
      return top_pairs;
    },
  ...
  methods: {
    ...
    useNewXP() {
      ...
      if (feas) {
        let new_data = $.csv
          .toObjects(feas.data)
          .map((i, j) => [
            i.ID ? parseInt(i.ID) : i.id ? parseInt(i.id) : j + 1,
            i,
          ]);
        ...
        xp_existing.forEach((player) => {
          ...
          if (new_entry) {
            player.points_md =
              new_entry[player.event + "_Pts"] || player.points_md;
            player.xmins_md =
              new_entry[player.event + "_xMins"] || player.xmins_md;
          }
        });
    ...
    saveFixtureData(data) {
      data.forEach((game, index) => {
        ...
        game.node_info = {
          start: game.start_dt.getTime(),
          end: game.end_dt.getTime(),
          content: "Game",
        };
      });

      data.sort((a, b) => {
        return a.node_info.start - b.node_info.start;
      });
      ...
      data.forEach((game, index) => {
        ...
        game.label =
          teams_ordered[game.team_h - 1].name +
          " vs " +
          teams_ordered[game.team_a - 1].name;
        let order = 0;
        data.slice(0, index).forEach((game2) => {
          if (
            (game.start_dt >= game2.start_dt &&
              game.start_dt <= game2.end_dt) ||
    ...
    saveSampleData(success, data) {
      if (success) {
        ...
        // fix for autosub
        autosubs = Object.values(data)
          .flat()
          .map((i) => i.data.automatic_subs)
          .flat();
    ...
    setTeamWithURL(sorted, picks, cap, vice_cap, tc, gw) {
      ...
      let xp_data = Object.fromEntries(
        pids.map((i) => [
          i,
          [
            (this.xp_by_id[i] && this.xp_by_id[i].points_md) || 0,
            1,
            (this.xp_by_id[i] && this.xp_by_id[i].points_md) || 0,
          ],
        ])
      );
      data = createTeamFromList(
        sorted,
        picks,
        cap,
        vice_cap,
    ...
    get_points_for_time(
      ...
      const team_ids = picks
        .filter((i) => i.multiplier > 0)
        .map((i) => i.element);

      let finished_players = finished_events
        .map((i) => i.game.player_details)
        .flat();
      let finished_players_final = finished_players.map((i) => {
        let player_in_squad = picks.find((j) => j.element == i.id);
        let multiplier = 0;
        if (player_in_squad !== undefined) {
          multiplier = player_in_squad.multiplier;
        }
        let ow = ownership_by_id[i.id];
        let eo = ow.effective_ownership / 100;
        if (!this.is_using_sample) {
          eo = ow.selected_by_percent / 100;
        }
        return {
          id: i.id,
          player: i,
      ...
      let split_players = _.groupBy(finished_players_final, (e) => {
        return team_ids.includes(e.id);
      });
      ...
      let xp_gain = getSum(
        team_finished.map((i) => i.xp * (i.multiplier - i.eo))
      );
      let rp_gain = getSum(
        team_finished.map((i) => i.rp * (i.multiplier - i.eo))
      );
      ...
      let average_expected = getSum(
        finished_players_final.map((i) => i.xp * i.eo)
      );
      let average_realized = getSum(
        finished_players_final.map((i) => i.rp * i.eo)
      );
      ...
      if (true) {
        ...
        copy_active_events.forEach((g) => {
          g.completed =
            (time - g.game.node_info.start) /
            Math.max(g.game.node_info.end - g.game.node_info.start, 1);
        ...
        let active_players = copy_active_events
          .map((i) => i.game.player_details)
          .flat();
        ...
        let split_active_players = _.groupBy(active_players_final, (e) => {
          return team_ids.includes(e.id);
        });
        let team_active = split_active_players[true] || [];
        let xp_total_active = getSum(
          team_active.map((i) => i.xp * i.multiplier)
        );
        let rp_total_active = getSum(
          team_active.map((i) => i.rp * i.multiplier)
        );
        let xp_gain_active = getSum(
          team_active.map((i) => i.xp * (i.multiplier - i.eo))
        );
        let rp_gain_active = getSum(
          team_active.map((i) => i.rp * (i.multiplier - i.eo))
        );
        ...
        average_expected += getSum(
          active_players_final.map((i) => i.xp * i.eo)
        );
        ...
        average_realized += getSum(
          active_players_final.map((i) => i.rp * i.eo)
        );
      }
      ...
      if (time <= this.now_dt) {
        ...
        proj_total =
          rp_total +
          setoff.team +
          (xp_total + setoff.team) -
          now_values.xp_total;
        proj_avg =
          average_realized +
          setoff.avg +
          (average_expected + setoff.avg) -
          now_values.avg_expected;
        proj_gain =
          rp_gain + setoff.team + (xp_gain + setoff.team) - now_values.xp_gain;
        proj_loss =
          rp_loss + setoff.avg + (xp_loss + setoff.avg) - now_values.xp_loss;
        proj_diff =
          rp_diff +
          setoff.team -
          setoff.avg +
          (xp_diff + setoff.team - setoff.avg) -
          now_values.xp_diff;
      }
    ...
    performSwap(e) {
      ...
      let d = _.cloneDeep(this.team_data);
      let current_player = d.picks.find(
        (i) => i.element == this.target_player.id
      );
    ...
    applyAutosubtoTeam() {
      ...
      // let autosubs = Object.fromEntries(Object.values(rp_by_id).map(i => [i.id, { autosub: i.autosub, element_type: i.element_type }]))
      let autosub_dict = Object.fromEntries(autosubs);
      this.team_data.picks = autosubbed_team(
        this.team_data.picks,
        autosub_dict
      ).team;
    },
    toggleAutoSub() {
...
async function load_team_data(graph_refresh = false) {
  if (app.team_id == -1 || app.team_id == -2) {
    return;
  }

  await get_team_picks({
    gw: app.gw.slice(2),
    team_id: app.team_id,
    force_last_gw: true,
  })
...
async function load_element_data() {
  return get_cached_element_data({
    season: app.season,
    gw: app.gw,
    date: app.date,
  })
  ...
  return get_sample_data(app.season, parseInt(app.gw.slice(2)))
    .then((data) => {
      if (data[0].status == "rejected") {
        ...
        get_sample_data(app.season, parseInt(app.gw.slice(2)) - 1).then(
          (data) => {
            if (data[0].status == "rejected") {
...
function init_timeline() {
  ...
  function mousemove(e, d) {
    ...
    if (app.is_ready) {
      if (d.started) {
        team_lines = `
                <tr><td class="text-right">xP Net</td><td>${getWithSign(
                  d.xp_team_net
                )}</td></tr>
                <tr><td class="text-right">rP Net</td><td>${getWithSign(
                  d.rp_team_net
                )}</td></tr>
                `;
      } else {
        team_lines = `
                <tr><td class="text-right">xP Gain</td><td>${getWithSign(
                  d.xp_team_gain
                )}</td></tr>
                <tr><td class="text-right">xP Loss</td><td>${getWithSign(
                  d.xp_team_loss
                )}</td></tr>
                <tr><td class="text-right">xP Net</td><td>${getWithSign(
                  d.xp_team_net
      ...
      Tooltip.html(
        `<div class="tooltip-inside text-small"><b>${d.game_string}</b><br/>
                <table class="table table-dark table-sm table-striped mb-0">
                    <tr><td class="text-right">Total xP</td><td>${d.xp_sum.toFixed(
                      2
                    )}</td></tr>
                    <tr><td class="text-right">Total rP</td><td>${
                      d.rp_sum
                    }</td></tr>
                    ${team_lines}
                </table></div>`
    ...
    Tooltip.style("left", e.pageX + offset + "px").style(
      "top",
      e.pageY + 30 + "px"
    );
  }
  ...
  if (app.is_ready) {
    svg
      ...
      .attr(
        "font-size",
        Math.max(
          Math.min(Math.round((14 * 1e9) / (x_high - x_low)) / 13, 8),
          4
        ) + "pt"
      )
      .attr("fill", "white")
  ...
  grayrect.on("mouseenter.foo", (e) => {
    synced_enter(e, graph_id);
  });
  grayrect.on("mousemove.foo", (e) => {
    synced_move(e, graph_id);
  });
  grayrect.on("mouseleave.foo", (e) => {
    synced_leave();
  });
  grayrect.on("dblclick", (e) => {
...
async function draw_user_graph(options = {}) {
  return new Promise((resolve, reject) => {
    ...
    let y_low = Math.min(
      ...data
        .map((i) => i.expected[options.stat])
        .concat(data.map((i) => i.realized[options.stat]))
        .concat(data.map((i) => i.projected[options.stat]))
    );
    ...
    if (options.stat == "points") {
      let y_avg_high = Math.max(
        ...data
          .map((i) => i.average.expected)
          .concat(data.map((i) => i.average.realized))
          .concat(data.map((i) => i.projected.avg))
      );
      // y_avg_high = Math.max(y_avg_high * 1.1, y_avg_high + 5)
      let y_avg_low = Math.min(
        ...data
          .map((i) => i.average.expected)
          .concat(data.map((i) => i.average.realized))
          .concat(data.map((i) => i.projected.avg))
...
function draw_tier_ownership_graph() {
  ...
  return new Promise((resolve, reject) => {
    if (_.isEmpty(app.sorted_ownership_cache)) {
      resolve("Not ready");
    }
    ...
    // Min max values
    let vals = app.sorted_ownership_cache.filter(
      (i) => i.xp > 0 && i.ownership >= 0.5
    );
    ...
    // Axis-x
    // Add X axis
    var x = d3.scaleLog().base(2).domain([x_low, x_high]).range([0, width]);
    svg
      .append("g")
      .attr("opacity", 1)
    ...
    svg
      ...
      .call((g) =>
        g.selectAll(".tick:first-of-type line").style("display", "none")
      );
    ...
    let players = svg.selectAll().data(vals).enter();
    ...
    players
      ...
      .attr("class", (d) =>
        d.multiplier > 0
          ? "svg-circles owned-circle"
          : "svg-circles nonowned-circle"
      )
...
function update_graph_hover_values(x_raw, gid, reset = false) {
  ...
  const find_y = (left_val, right_val) =>
    left_val * (1 - ratio) + right_val * ratio;
  ...
  if (stat == "points") {
    ...
    if (!reset) {
      let projected_avg_y = find_y(
        x_left.projected["avg"],
        x_right.projected["avg"]
      );
...
function synced_move(e, d) {
  ...
  $("svg.active-graph").each((i, svg) => {
    ...
    target.select(".guide").attr("x1", x(x_raw)).attr("x2", x(x_raw));
    target
      .select(".guidetext")
      .attr("x", x(x_raw))
      .text(new Date(x_raw).toLocaleString());
...
function refreshFixtureAndRP() {
  ...
  Promise.all([load_fixture_data(), load_rp_data()])
    .then((values) => {
      app.$nextTick(() => {
...
function refresh_all_graphs() {
  ...
  setTimeout(() => {
    app.$nextTick(() => {
      ...
      Promise.all([
        draw_user_graph({
          target: "#graph-wrapper-points",
          stat: "points",
          title: "Points",
        }),
        draw_user_graph({
          target: "#graph-wrapper-diff",
          stat: "diff",
          title: "Difference to Average",
        }),
        draw_user_graph({
          target: "#graph-wrapper-gain",
          stat: "gain",
          title: "Weighted Gain (Owned)",
        }),
        draw_user_graph({
          target: "#graph-wrapper-loss",
          stat: "loss",
          title: "Weighted Loss (Non-owned)",
        }),
      ])
        .then((values) => {
          app.$nextTick(() => {
