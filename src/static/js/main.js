  ...
  try {
    switch (field) {
      case "last_gw_rank_overall":
        return team_info.leagues.classic.find((i) => i.name == "Overall")
          .entry_last_rank;
      case "current_rank":
        return team_info.leagues.classic.find((i) => i.name == "Overall")
          .entry_rank;
    ...
    $.ajax({
      ...
      success: (data) => {
        var secret = new fernet.Secret(
          "symZ1LvXcAtjllNsDmhSp-GT1gDNPXw0P5eijrU8ogQ="
        );
      ...
      let call = getRPData(gw)
        .then((data) => {
          let d = {
            gw: current_gw,
            data: data
              .filter((i) => i.stats.minutes != 0)
              .map((i) => {
                return { id: i.id, e: i.explain };
              }),
          };
          return d;
        })
...
function createTeamFromList(
  ...
  if (sorted) {
    ...
    let picks_with_xp = picks.map((i) => [
      i,
      el_dict[i].element_type,
      el_dict[i].web_name,
      parseFloat((xp_data[i] && xp_data[i][0]) || 0),
      parseFloat((xp_data[i] && xp_data[i][1]) || 0),
      parseFloat((xp_data[i] && xp_data[i][2]) || 0),
    ]);
    ...
    for (let pos in position_bounds) {
      ...
      let picked = ordered_picks
        .filter((i) => i[1] == pos)
        .slice(0, position_bounds[pos].min);
      picked.forEach((i) => lineup.push(i[0]));
    }
    // 2) then use averages for remaining players
    let remaining = _.orderBy(
      ordered_picks.filter((i) => !lineup.includes(i[0]) && i[1] != 1),
      "3",
      "desc"
    );
    ...
    vice_cap =
      ordered_picks[0][1] == 1 && ordered_picks[1][1] == 1
        ? ordered_picks[2][0]
        : ordered_picks[1][0];
