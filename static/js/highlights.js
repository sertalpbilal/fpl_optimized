...
var app = new Vue({
  ...
  computed: {
    is_ready() {
      return (
        this.ready && !_.isEmpty(this.team_data) && !_.isEmpty(this.points_data)
      );
    },
    completed() {
    ...
    stats_2d() {
      ...
      let structured_data = Object.entries(this.points_data).map((i) =>
        i[1].map((j) => [i[0], j.id, j.e])
      );
    ...
    gw_pid_pts() {
      ...
      Object.keys(gw_grouped).map((key, index) => {
        gw_grouped[key] = Object.fromEntries(
          gw_grouped[key].map((i) => [i.id, i.pts])
        );
      });
    ...
    pid_gw_pts() {
      ...
      Object.keys(id_grouped).map((key, index) => {
        id_grouped[key] = Object.fromEntries(
          id_grouped[key].map((i) => [i.gw, i.pts])
        );
      });
      return id_grouped;
    },
    ...
    user_player_stats() {
      ...
      let picks = Object.entries(this.team_data)
        .map((i) => i[1].picks.map((j) => ({ gw: i[0], ...j })))
        .flat();
      picks.forEach((p) => {
    ...
    user_player_sum() {
      ...
      Object.keys(grouped_pick_data).map((key) => {
        ...
        e["lineup_count"] = grouped_pick_data[key].filter(
          (i) => i.multiplier > 0
        ).length;
        e["points_total"] = grouped_pick_data[key]
          .map((i) => (i.points || 0) * i.multiplier)
          .reduce((a, b) => a + b, 0);
        let pt_total = grouped_pick_data[key]
          .map(
            (i) =>
              (i.points || 0) *
              (i.multiplier - (eo_dict[i.gw + "_" + i.element] || 0) / 100)
          )
          .reduce((a, b) => a + b, 0);
        e["points_total_rel"] = _.round(pt_total, 1);
        e["cap_count"] = grouped_pick_data[key].filter(
          (i) => i.multiplier > 1
        ).length;
      ...
      let sorted_data = _.orderBy(
        grouped_all_stats,
        ["points_total", "squad_count", "lineup_count"],
        ["desc", "desc", "desc"]
      );
      return sorted_data;
    },
    ...
    toty_final() {
      ...
      if (this.toty_relative) {
        return this.user_toty_relative;
      } else {
        return this.user_toty;
      }
    },
    user_toty() {
      if (!this.is_ready) {
      ...
      for (let pos = 1; pos < 5; pos++) {
        ...
        while (lineup_count[pos] < min_target) {
          for (let i = 0; i < player_sum_data.length; i++) {
            ...
            if (lineup_count[pos] >= min_target) {
              break;
            }
          }
          break;
      ...
      for (let i = 0; i < player_sum_data.length; i++) {
        let p = player_sum_data[i];
        let pos = p["position"];
        if (selected_index[p.id]) {
          continue;
        }
        if (
          lineup_count[pos] < element_type[pos].max &&
          getSum(Object.values(lineup_count)) < 11
        ) {
          selected_players.push({
            player: p,
            lineup: true,
      ...
      selected_players = _.orderBy(
        selected_players,
        ["is_keeper", "points_total"],
        ["desc", "desc"]
      );
    ...
    user_picks_detailed() {
      ...
      let all_team_picks = Object.keys(team_data)
        .map((i) => team_data[i].picks.map((j) => ({ gw: i, ...j })))
        .flat();
      let pick_mult_map = all_team_picks.map((i) => [
        i.gw + "," + i.element,
        i.multiplier,
      ]);
      let pick_mult = Object.fromEntries(pick_mult_map);
      let picked_stats = stat_detailed.filter(
        (i) => i.gw + "," + i.id in pick_mult
      );
    ...
    user_picks_with_order() {
      ...
      if (_.isEmpty(this.user_ownership_gain_loss)) {
        return [];
      }
      let td = _.cloneDeep(this.team_data);
      let picks = Object.keys(td)
        .map((i) => td[i].picks.map((j) => ({ gw: parseInt(i), ...j })))
        .flat();
      // _.cloneDeep(this.user_ownership_gain_loss.picked_players)
      let chips = Object.fromEntries(
        Object.entries(td).map((i) => [i[0], i[1].active_chip])
      );
      let fpl_el = this.fpl_element;
      let rp_dict = this.pid_gw_pts;

      picks.forEach((el) => {
        el.element_type = fpl_el[el.element].element_type;
      });
      picks.forEach((el) => {
        el.rp = _.get(rp_dict, el.element + "." + el.gw) || 0;
      });
      ...
      gws.forEach((gw) => {
        ...
        eltypes.forEach((pos) => {
          ...
          // existing ones
          let existing_players = pos_picks.filter((i) =>
            prev_picks.map((i) => i.element).includes(i.element)
          );
          ...
          // new players
          let new_players = _.differenceWith(
            pos_picks,
            existing_players,
            _.isEqual
          );
          new_players.forEach((el) => {
            ...
            let posssible = _.range(start_order, finish_order + 1).filter(
              (i) => _.get(gw_order_dict, gw + "." + i) == undefined
            );
    ...
    pick_stats() {
      if (_.isEmpty(this.user_picks_with_order)) {
        return {};
      }
    ...
    user_grouped_by_fixture() {
      return _(this.user_picks_detailed)
        .groupBy("id")
        .map((i, v) => [v, _(i).groupBy("fixture").value()])
        .value();
    },
    user_picks_origin() {
      if (!this.is_ready) {
      ...
      gameweeks.forEach((gw) => {
        ...
        // }
          ...
          let target_gw =
            data[gw - 1].active_chip == "freehit" ? gw - 2 : gw - 1;
      ...
      let all_players = Object.values(data)
        .map((i) => i.picks)
        .flat();

      let age_pts = _.map(_.groupBy(all_players, "age"), (val, key) => [
        key,
        _.sumBy(val, "eff_points"),
      ]);
      ...
      stats = {
        ...
        pts_main: {
          initial: _.sum(
            all_players
              .filter((i) => i.origin == "initial")
              .map((i) => i.eff_points)
          ),
          wildcard: _.sum(
            all_players
              .filter((i) => i.origin == "wildcard")
              .map((i) => i.eff_points)
          ),
          freehit: _.sum(
            all_players
              .filter((i) => i.origin == "freehit")
              .map((i) => i.eff_points)
          ),
          transfer: _.sum(
            all_players
              .filter((i) => i.origin == "transfer")
              .map((i) => i.eff_points)
          ),
        },
        // 'split': {
        ...
        pts_details: {
          ...
          wc1: _.sum(
            all_players
              .filter((i) => i.origin == "wildcard" && i.wildcard_cnt == 1)
              .map((i) => i.eff_points)
          ),
          wc2: _.sum(
            all_players
              .filter((i) => i.origin == "wildcard" && i.wildcard_cnt == 2)
              .map((i) => i.eff_points)
          ),
          // 'wcb': _.sum(all_players.filter(i => i.origin == 'wildcard_bonus').map(i => i.eff_points)),
          fh1: _.sum(
            all_players
              .filter((i) => i.origin == "freehit" && i.freehit_cnt == 1)
              .map((i) => i.eff_points)
          ),
        ...
        groups: {
          ...
          wc1: all_players.filter(
            (i) => i.origin == "wildcard" && i.wildcard_cnt == 1
          ),
          wc2: all_players.filter(
            (i) => i.origin == "wildcard" && i.wildcard_cnt == 2
          ),
          // 'wcb': all_players.filter(i => i.origin == 'wildcard_bonus'),
          fh1: all_players.filter(
            (i) => i.origin == "freehit" && i.freehit_cnt == 1
          ),
      ...
      let main_coll = (m = _(all_players)
        .groupBy("origin")
        .mapValues((val, key) => _(val).groupBy("age_group").value())
        .value());

      let main_sum = _(m)
        .mapValues((val, key) =>
          _(val)
            .mapValues((ival, ikey) => _.sumBy(ival, "eff_points"))
            .value()
        )
        .value();
      let main_cnt = _(m)
        .mapValues((val, key) =>
          _(val)
            .mapValues(
              (ival, ikey) => ival.filter((i) => i.multiplier > 0).length
            )
            .value()
        )
        .value();
      ...
      let nested_collection = (e = _(all_players)
        .groupBy("origin_text")
        .mapValues((val, key) => _(val).groupBy("age_group").value())
        .value());
      let age_groups = _.uniq(
        _(e)
          .map((val, key) =>
            _(val)
              .map((ival, ikey) => ikey)
      ...
      let age_sums = _(all_players)
        .groupBy("age_group")
        .mapValues((val, key) => _(val).sumBy("eff_points"))
        .value();
      let age_cnt = _(all_players)
        .groupBy("age_group")
        .mapValues((val, key) => val.filter((i) => i.multiplier > 0).length)
        .value();

      let grp_cnt = _(all_players)
        .groupBy("origin_text")
        .mapValues((val, key) => val.filter((i) => i.multiplier > 0).length)
        .value();
    ...
    user_stats_per_gw() {
      ...
      let stat_dict = Object.fromEntries(
        stats.map((i) => [[i.key, i.gw, i.id].join("_"), i])
      );
      // let picks = this.team_data.map(i => i.picks)
      let user_picks = this.user_player_stats;
      let fpl_data = this.fpl_element;
      user_picks = user_picks
        .map((i) => ({ gw: i.gw, id: i.element, multiplier: i.multiplier }))
        ...
        for (let pick of user_picks) {
          ...
          if (m != undefined) {
            picked_stats.push({
              ...
              total_points: pick.multiplier * m.points,
              eltype: fpl_data[pick.id].element_type,
            });
          }
        }
      }
      ...
      let gw_grouped = Object.fromEntries(
        _(picked_stats)
          .groupBy("gw")
          .map((i, k) => [k, _.groupBy(i, "key")])
          .value()
      );
      ...
      let pt_data = Object.entries(this.team_data).map((i) => ({
        gw: i[0],
        net_pts:
          i[1].entry_history.points - i[1].entry_history.event_transfers_cost,
        pts: i[1].entry_history.points,
        hit_pts: i[1].entry_history.event_transfers_cost,
      }));
      for (let pgw of pt_data) {
        ...
        for (let key in gw_grouped[gw]) {
          ...
          type_names[key] = t
            .map(
              (ev) =>
                this.fpl_element[ev.id].web_name +
                (ev.value > 1 ? ` (${ev.value})` : "")
            )
            .join(", ");
        }
    ...
    user_player_gws() {
      ...
      let all_picks = Object.entries(team)
        .map((i) =>
          i[1].picks.map((j) => ({
            gw: i[0],
            id: j.element,
    ...
    user_points_by_eltype() {
      ...
      let values = stats.map((i) => ({
        id: i.element,
        eltype: i.raw.element_type,
        cost: i.raw.now_cost,
        total_points: (i.points || 0) * i.multiplier,
      }));
      ...
      let arr_vals = Object.keys(grouped_vals)
        .map((eltype) =>
          Object.keys(grouped_vals[eltype]).map((price_cat) => [
            element_type[eltype].short,
            price_cat,
            grouped_vals[eltype][price_cat],
          ])
        )
        .flat();
    ...
    user_formation_analysis_by_eltype() {
      ...
      for (let p in element_type) {
        ...
        row["picked"] = _.uniqWith(
          pos_select,
          (a, b) => a.gw == b.gw && a.id == b.id
        ).length;
        row["games"] = _.uniqWith(
          pos_select,
          (a, b) => a.id == b.id && a.fixture == b.fixture
        ).length;
    ...
    user_formation_analysis_by_gw() {
      ...
      for (let gw of gameweeks) {
        ...
        row["formation"] =
          ...
          cnt[4];
        row["points"] = getSum(
          stats.filter((i) => i.gw == gw).map((i) => i.total_points)
        );
    ...
    parsed_eo_data() {
      ...
      for (let gw of Object.keys(eo_data)) {
        if (key in eo_data[gw]) {
          let v = Object.keys(eo_data[gw][key])
            .filter((i) => i != "meta")
            .map((pid) => [gw, pid, eo_data[gw][key][pid].eo]);
          values.push(v);
          let h = eo_data[gw][key]["meta"];
          hits.push(h);
        } else {
          let v = Object.keys(eo_data[gw]["Overall"])
            .filter((i) => i != "meta")
            .map((pid) => [gw, pid, eo_data[gw]["Overall"][pid].eo]);
          values.push(v);
      ...
      let values_dict = Object.fromEntries(
        values.map((i) => [i[0] + "_" + i[1], i[2]])
      );
    ...
    user_ownership_gain_loss() {
      ...
      let all_pairs = Object.fromEntries(
        gws
          .map((gw) =>
            all_pids.map((pid) => [gw + "_" + pid, { gw: gw, id: pid }])
          )
          .flat()
      );
      ...
      let player_gains = _(all_data)
        ...
        .map((i, v) => {
          ...
          return {
            ...
            loss,
            net: gain - loss,
            magnitude: Math.abs(gain - loss),
            points: pts,
            xgain,
            xloss,
            xnet: xgain - xloss,
            xmagnitude: Math.abs(xgain - xloss),
            xpoints: xpts,
            diff,
            xdiff,
            luck,
            luckmagnitude: Math.abs(luck),
          };
        })
        .value();
      player_gains = _(player_gains).orderBy(["magnitude"], ["desc"]).value();
      let max_impact = player_gains[0].magnitude;
      player_gains.forEach((e) => {
        e.impact = (e.magnitude / max_impact) * 100;
      });

      player_xgains = _(_.cloneDeep(player_gains))
        .orderBy(["xmagnitude"], ["desc"])
        .value();
      let exp_max_impact = player_xgains[0].xmagnitude;
      player_xgains.forEach((e) => {
        e.ximpact = (e.xmagnitude / exp_max_impact) * 100;
      });

      player_luck = _(_.cloneDeep(player_gains))
        .orderBy(["luckmagnitude"], ["desc"])
        .value();
      let max_luck = player_luck[0].luckmagnitude;
      player_luck.forEach((e) => {
        e.luckimpact = (e.luckmagnitude / max_luck) * 100;
      });
      ...
      for (let gw_entry of combined) {
        ...
        if (!groups.hasOwnProperty("false")) {
          groups["false"] = [];
        }
        if (!groups.hasOwnProperty("true")) {
          groups["true"] = [];
        }
        ...
        let hit_gain =
          gw_field_hits.hit_total / Math.max(1, gw_field_hits.teams);
        let gain_with_hit = gain + hit_gain;
        let hit_loss =
          _.get(user_team[gw_entry[0]], "entry_history.event_transfers_cost") ||
          0;
        ...
        let top_bets_for = _.orderBy(gw_picks, ["own_rate"], ["desc"]).slice(
          0,
          3
        );
        let top_bets_against = _.orderBy(
          gw_entry[1].filter((i) => !i.owned),
          ["eo"],
          ["desc"]
        ).slice(0, 3);
        let net = gain + loss;
      ...
      all_pairs = all_pairs.filter(
        (i) => i.eo > 2 && i.total_pts > 3 && i.net <= -1
      );
      return {
        picked_players: user_picks,
        gains: user_picks,
        losses: all_pairs,
        combined_per_gw: combined,
        combined_per_player: player_gains,
        combined_per_player_exp: player_xgains,
        comb_per_player_luck: player_luck,
      };
    },
    ...
    team_fdr_values() {
      ...
      let c = (r) =>
        d3
          .scaleLinear()
          .domain([0, 0.5, 1])
          .range(["#67ADB2", "#999898", "#C85454"])(r);
      teams.forEach((team, order) => {
        team.offense_ratio =
          (team.offense - offense_min) / (offense_max - offense_min);
        team.defense_ratio =
          (defense_max - team.defense) / (defense_max - defense_min);
    ...
    user_fdr_values() {
      ...
      let picks = this.user_picks_detailed.filter(
        (i) => i.identifier == "minutes" && i.multiplier > 0
      );
      let fdr_values = this.team_fdr_values;
      if (_.isEmpty(fdr_values)) {
        return [];
      }
      let fpl_data = this.fpl_element;
      ...
      picks.forEach((p) => {
        ...
        let pf = (p.player_fdr =
          p.eltype <= 2 ? fixture_fdr.offense : fixture_fdr.defense);

        p.player_fdr_ratio =
          p.eltype <= 2 ? fixture_fdr.offense_ratio : fixture_fdr.defense_ratio;
        ...
        p.self_team_fdr =
          p.eltype <= 2 ? p.self_team.defense_ratio : p.self_team.offense_ratio;

        p.total = getSum(
          total_points_dict[p.id][p.fixture].map((i) => i.points)
        );
      });

      picks = _(picks)
        .orderBy(
          ["player_fdr_ratio", "self_team_fdr", "total"],
          ["asc", "desc", "desc"]
        )
        .value();
    ...
    player_xp_dict() {
      ...
      return Object.fromEntries(
        e.map((i) => [i.gw + "_" + i.ID, _.round(parseFloat(i.xp), 2)])
      );
    },
    player_gw_reference() {
      if (_.isEmpty(this.xp_data)) {
    ...
    user_gw_results() {
      if (_.isEmpty(this.team_data) || _.isEmpty(this.xp_data)) {
        return;
      }
      let xp_dict = this.player_xp_dict;
      let rp_dict = this.pid_gw_pts;
      ...
      let gw_results = {};
      let total_exp_diff = 0,
        total_real_diff = 0,
        total_xp = 0,
        total_rp = 0,
        total_field_xp = 0,
        total_field_rp = 0,
        total_exp_real_diff = 0;

      gws.forEach((gw) => {
        let gw_picks = this.team_data[gw].picks;
        let gw_hit = this.include_hits
          ? _.get(this.team_data[gw], "entry_history.event_transfers_cost") || 0
          : 0;
        let gw_field_hits = eo_dict.hits[gw - 1];
        if (gw_field_hits == undefined) {
          gw_field_hits = { hit_total: 0, teams: 0 };
        }
        let field_hit = this.include_hits
          ? gw_field_hits.hit_total / Math.max(1, gw_field_hits.teams)
          : 0;
        ...
        let xp =
          _.round(
            _.sum(
              gw_picks.map(
                (i) =>
                  i.multiplier * (_.get(xp_dict, gw + "_" + i.element) || 0)
              )
            ),
            2
          ) - gw_hit;
        let rp =
          _.sum(
            gw_picks.map(
              (i) => i.multiplier * (_.get(rp_dict, i.element + "." + gw) || 0)
            )
          ) - gw_hit;
        ...
        let field_xp =
          _.round(
            _.sum(
              el_ids.map(
                (i) =>
                  ((_.get(eo_dict, "dict." + gw + "_" + i) || 0) / 100) *
                  (_.get(xp_dict, gw + "_" + i) || 0)
              )
            ),
            2
          ) - field_hit;
        ...
        let skill_ratio = _.round(
          (100 * exp_diff) / (Math.abs(exp_diff) + Math.abs(luck)),
          1
        );
        let luck_ratio = _.round(
          (100 * luck) / (Math.abs(exp_diff) + Math.abs(luck)),
          1
        );

        let total_skill_ratio = _.round(
          (100 * total_exp_diff) /
            (Math.abs(total_exp_diff) + Math.abs(total_exp_real_diff)),
          1
        );
        let total_luck_ratio = _.round(
          (100 * total_exp_real_diff) /
            (Math.abs(total_exp_diff) + Math.abs(total_exp_real_diff)),
          1
        );
    ...
    calculated_luck() {
      if (_.isEmpty(this.final_outcome)) {
        return undefined;
      }
    ...
    final_outcome() {
      if (_.isEmpty(this.user_gw_results)) {
        return undefined;
      }
    ...
    transfer_analysis() {
      ...
      let xp_sum = (el, weeks) =>
        _.sum(weeks.map((i) => xp_val(el, i.gw) * i.mult));
      let rp_sum = (el, weeks) =>
        _.sum(weeks.map((i) => rp_val(el, i.gw) * i.mult));
      let find_best_xp = (el_type, cost, weeks, exclude, target_week) => {
        ...
        let candidates = el_data
          .filter((i) => i.element_type == el_type)
          .filter((i) => gw_ref[target_week + "_" + i.id])
          .filter(
            (i) =>
              parseFloat(gw_ref[target_week + "_" + i.id].price) <=
              parseFloat(cost)
          )
          .filter((i) => !exclude.includes(i.id));
        let totals = candidates.map((i) => {
          return { ...i, xp_total: xp_sum(i.id, weeks) };
        });
      ...
      let find_best_rp = (el_type, cost, weeks, exclude, target_week) => {
        ...
        let candidates = el_data
          ...
          .filter((i) => !exclude.includes(i.id));
        let totals = candidates.map((i) => {
          return { ...i, rp_total: rp_sum(i.id, weeks) };
        });
      ...
      all_gws.forEach((w) => {
        ...
        let sold = _.cloneDeep(
          _.difference(last_gw_team, this_gw_team).map((i) => elements[i])
        );
        let bought = _.cloneDeep(
          _.difference(this_gw_team, last_gw_team).map((i) => elements[i])
        );
        let gw_transfer = [];
        sold.forEach((p) => {
          if (p.element_type == 5) {
          ...
          let match = bought.find(
            (i) => i.used == undefined && i.element_type == p.element_type
          );
          match.used = true;
          let new_p = match.id;
          // check future GWs for new player
          future_five = _.range(w, w + 5);
          let future_plays = future_five
            .filter((j) => team_data[j])
            .map((j) => {
              return {
                gw: j,
                play: team_data[j].picks.find((k) => k.element == new_p),
              };
            })
            .filter((j) => j.play != undefined)
            .filter((j) => j.play.multiplier > 0);
          future_plays.forEach((f) => {
            f.mult = f.play.multiplier;
          });
          let bought_player_plays = future_plays.map((i) => {
            return {
              ...i,
              xp: xp_dict[i.gw + "_" + new_p] || 0,
              rp: rp_val(new_p, i.gw),
            };
          });
          let sold_player_plays = future_plays
            .map((i) => {
              return { gw: i.gw, mult: i.play.multiplier };
            })
            .map((i) => {
              return {
                ...i,
                xp: xp_dict[i.gw + "_" + p.id] || 0,
                rp: rp_val(p.id, i.gw),
              };
            });

          let sold_xp_total = _.sum(
            Object.values(sold_player_plays).map((i) => i.xp * i.mult)
          );
          let bought_xp_total = _.sum(
            Object.values(bought_player_plays).map((i) => i.xp * i.mult)
          );
          let xp_diff = bought_xp_total - sold_xp_total;

          let sold_rp_total = _.sum(
            Object.values(sold_player_plays).map((i) => i.rp * i.mult)
          );
          let bought_rp_total = _.sum(
            Object.values(bought_player_plays).map((i) => i.rp * i.mult)
          );
          let rp_diff = bought_rp_total - sold_rp_total;

          let gw_list = future_plays
            .map((i) =>
              i.play.multiplier > 1
                ? `${i.gw} (${i.play.multiplier == 2 ? "C" : "TC"})`
                : i.gw
            )
            .join(", ");
          ...
          let best_xp = find_best_xp(
            elements[p.id].element_type,
            gw_ref?.[w + "_" + match.id]?.price,
            future_plays,
            last_gw_team,
            w
          );
          if (best_xp != undefined) {
            ...
            if (best_xp.xp_total == 0) {
              xp_ratio_raw = 100;
            }
            xp_ratio = rounded(xp_ratio_raw, 1) + "%";
          }
          let best_rp = find_best_rp(
            elements[p.id].element_type,
            gw_ref[w + "_" + match.id]?.price,
            future_plays,
            last_gw_team,
      ...
      let all_tr = Object.values(analysis.gws)
        .flat()
        .filter((i) => i.best_xp != undefined);
      analysis.fs_ratio =
        rounded(
          (all_tr.filter((i) => i.fs_optimal).length / all_tr.length) * 100,
          1
        ) + "%";
      analysis.hs_ratio =
        rounded(
          (all_tr.filter((i) => i.hs_optimal).length / all_tr.length) * 100,
          1
        ) + "%";
      ...
      analysis.tr_optimality =
        _.sum(all_tr.map((i) => i.xp_ratio_raw)) / Math.max(all_tr.length, 1);
  ...
  methods: {
    fetch_team_picks() {
      ...
      let init_call = get_team_info(this.team_id)
        .then((response) => {
        ...
        .catch((e) => {
          ...
          let cached_info = JSON.parse(localStorage.getItem("team_info"));
          let team_cached_info = _.get(
            cached_info,
            app.season + ".T" + app.team_id
          );
      ...
      if (stored_team_picks == null) {
        ...
        _.get(stored_team_picks, this_season + ".T" + app.team_id) == undefined
      ) {
        _.set(stored_team_picks, this_season + ".T" + app.team_id, {});
      }
      ...
      for (gw = 1; gw <= this.max_gw; gw++) {
        ...
        if (
          app.caches_enabled &&
          current_gw != app.next_gw &&
          season_picks != undefined &&
          current_gw in season_picks
        ) {
    ...
    refresh_table() {
      ...
      this.top_players_table.buttons().container().appendTo("#csv_buttons");
          ...
          this.api()
            ...
            .every(function () {
              ...
              column
                ...
                .unique()
                .sort((a, b) =>
                  parseInt(a) ? parseInt(a) - parseInt(b) : a > b ? 1 : -1
                )
                .each(function (d, j) {
      ...
      this.all_picks_table.buttons().container().appendTo("#csv_buttons3");
      ...
      tg_table.buttons().container().appendTo("#csv_buttons4");
      ...
      tge_table.buttons().container().appendTo("#csv_buttons_exp");
      ...
      tgl_table.buttons().container().appendTo("#csv_buttons_luck_table");
      ...
      fdr_table.buttons().container().appendTo("#csv_buttons5");
    },
    add_ID_to_compare() {
...
function get_team_stats_picks(picks) {
  ...
  let clean_sheets = picked_stats.filter(
    (i) => i.eltype <= 2 && i.identifier == "clean_sheets" && i.multiplier > 0
  );
  let defense_picks = _.cloneDeep(
    picked_stats.filter(
      (i) => i.eltype <= 2 && i.identifier == "minutes" && i.multiplier > 0
    )
  );
  for (let p of defense_picks) {
    let match = clean_sheets.filter(
  ...
  let cs_stat = {
    count: clean_sheets.length,
    total: defense_picks.length,
    values: defense_picks,
    info: "Rate of CS gains out of all GK and DF",
  };

  let goal_count = picked_stats.filter(
    (i) => i.eltype >= 3 && i.identifier == "goals_scored" && i.multiplier > 0
  );
  let attack_picks = _.cloneDeep(
    picked_stats.filter(
      (i) => i.eltype >= 3 && i.identifier == "minutes" && i.multiplier > 0
    )
  );
  for (let p of attack_picks) {
    let match = goal_count.filter(
      (i) => i.id == p.id && i.gw == p.gw && i.fixture == p.fixture
    );
    if (match.length != 0) {
      p.success = true;
  ...
  let goal_stat = {
    count: goal_count.length,
    total: attack_picks.length,
    values: attack_picks,
    info: "Rate of goal returns out of all MD and FW",
  };

  let assists_count = picked_stats.filter(
    (i) => i.eltype >= 3 && i.identifier == "assists" && i.multiplier > 0
  );
  let assist_attack_picks = _.cloneDeep(
    picked_stats.filter(
      (i) => i.eltype >= 3 && i.identifier == "minutes" && i.multiplier > 0
    )
  );
  for (let p of assist_attack_picks) {
    let match = assists_count.filter(
      (i) => i.id == p.id && i.gw == p.gw && i.fixture == p.fixture
    );
    if (match.length != 0) {
      p.success = true;
  ...
  let assist_stat = {
    count: assists_count.length,
    total: assist_attack_picks.length,
    values: assist_attack_picks,
  ...
  let all_count = _.cloneDeep(
    picked_stats.filter((i) => i.identifier == "minutes" && i.multiplier > 0)
  );
  for (let p of all_count) {
    let match = bonus_count.filter(
      (i) => i.id == p.id && i.gw == p.gw && i.fixture == p.fixture
    );
    if (match.length != 0) {
      p.success = true;
  ...
  let bonus_stat = {
    count: bonus_count.length,
    total: all_count.length,
    values: all_count,
    info: "Rate of bonus returns out of all players",
  };

  let captain_pts = picked_stats.filter((i) => i.multiplier > 1);
  let captain_game_pts = {};
  captain_pts.forEach((c) => {
    if (!(c.fixture in captain_game_pts)) {
      captain_game_pts[c.fixture] = 0;
    }
    captain_game_pts[c.fixture] += c.total_points;
  });
  let non_blank_captain = Object.values(captain_game_pts).filter((i) => i > 7);
  let captain_count = _.cloneDeep(
    picked_stats.filter((i) => i.multiplier > 1 && i.identifier == "minutes")
  );
  ...
  let captain_stat = {
    count: non_blank_captain.length,
    total: captain_count.length,
    values: captain_count,
    info: "Rate of non-blanking (7+ pts) captain returns",
  };
...
function export_all_data() {
  let raw_data = Object.entries(app.$data).filter(
    (i) => !i[0].includes("_table")
  );
  let comp_data = Object.fromEntries(
    Object.keys(app.$options.computed).map((i) => [i, app[i]])
  );
  let obj = { ...raw_data, ...comp_data };
  downloadToFile(
    JSON.stringify(obj, undefined, 2),
    "myFPLdata_extended.json",
    "application/json"
  );
}
...
function download_excel() {
  ...
  let tinfo = Object.entries(tinfo_raw).map((i) => {
  ...
  let ent = Object.entries(td).map((i) => {
    return { gw: i[0], ...i[1] };
  });
  ent.forEach((e) => {
    ...
    e["pick_names"] = e["picks"]
      .map((i) => app.fpl_element[i.element].web_name)
      .join(",");
    delete e.picks;
  });
  ...
  let ent3 = Object.entries(td3).map((i) => {
    return { gw: i[0], ...i[1] };
  });
  let p_picks = [];
  ...
  d4 = d4.map((i) => {
    return { name: app.fpl_element[i.id].web_name, ...i };
  });
  ...
  let ep_list = _.map(event_pts, (value, key) => {
  ...
  for (let ep of ep_list) {
    ...
    let per_player = player_stat_types[key].pp
      ? rounded(
          getSum(value.map((i) => parseInt(i.value))) /
            user_stats_per_gw.total_picks,
          2
        )
      : "-";
    let points = getSum(value.map((i) => parseInt(i.points * i.multiplier)));
    let percentage = rounded(
      (getSum(value.map((i) => parseInt(i.points * i.multiplier))) /
        user_stats_per_gw.overall_total) *
        100,
      1
    );
  ...
  let d11 = _(detailed_point_cache)
    .groupBy(1)
    .map((value, key) => {
      return {
        type: key,
        ..._.fromPairs(
          value.map((j) => [j[0] == "Total" ? j[0] : "GW" + j[0], j[2]])
        ),
      };
    })
    .value();
  ...
  let d12 = _.map(_.cloneDeep(app.user_picks_custom_stats), (value, key) => {
    return {
      target: key,
      count: value.count,
      picks: value.total,
      success_ratio: _.round((100 * value.count) / value.total, 2),
      info: value.info,
    };
  });
  ...
  let d14 = _.cloneDeep(app.user_ownership_gain_loss.gains).map((i) => {
    return { name: app.fpl_element[i.id].web_name, ...i };
  });
  let s14 = XLSX.utils.json_to_sheet(d14);
  XLSX.utils.book_append_sheet(wb, s14, "Gain Analysis");
  // sheet 15: loss analysis
  let d15 = _.cloneDeep(app.user_ownership_gain_loss.losses).map((i) => {
    return { name: app.fpl_element[i.id].web_name, ...i };
  });
...
function draw_player_bar_chart(div_id, id) {
  ...
  let lineup_gws = user_gws
    .filter((i) => i.id == id && i.multiplier > 0)
    .map((i) => i.gw);
  let bench_gws = user_gws
    .filter((i) => i.id == id && i.multiplier == 0)
    .map((i) => i.gw);
  let captain_gws = user_gws
    .filter((i) => i.id == id && i.multiplier > 1)
    .map((i) => [i.gw, i.multiplier, raw_data[i.gw]]);
  ...
  svg
    .append("g")
    .call(d3.axisRight(y).tickSize(width).tickValues(d3.range(min_y, max_y, 4)))
    .call((g) =>
      g
        .selectAll(".tick line")
  ...
  svg.call((s) =>
    s.selectAll(".tick").attr("font-size", "5pt").attr("fill", "white")
  );
...
function draw_type_heatmap() {
  ...
  let x = d3.scaleBand().range([0, width]).domain(xvals).padding(0.05);
  svg.append("g").call(d3.axisTop(x).tickSize(0)).select(".domain").remove();

  let y = d3.scaleBand().range([0, height]).domain(yvals).padding(0.05);
  ...
  let pos_totals = _(data)
    .groupBy(0)
    .map((i, v) => [v, _.sumBy(i, 2)])
    .value();
...
function draw_radar_map() {
  ...
  let data = [
    Object.entries(app.user_picks_custom_stats).map((i) => ({
      stat: i[0],
      value: (i[1].count / i[1].total) * 100,
    })),
  ];
  const names = [
    app.team_info.player_first_name + " " + app.team_info.player_last_name,
  ];
  ...
  axis
    ...
    // .enter()
    .data((d, j) =>
      d3.range(1, axisCircles + 1).map((i) => ({ i, d: d, j: j }))
    )
    .enter()
    .append("text")
    .attr(
      "x",
      (d) =>
        rScale((maxValue * d.i) / axisCircles) *
          Math.cos(angleSlice * d.j - Math.PI / 2) +
        center.x
    )
    ...
    .attr(
      ...
      (d) =>
        rScale((maxValue * d.i) / axisCircles) *
          Math.sin(angleSlice * d.j - Math.PI / 2) +
        center.y
    )
    .attr("alignment-baseline", "middle")
    .attr("dominant-baseline", "middle")
  ...
  plots
    ...
    .attr(
      "cx",
      (d, i) =>
        rScale(d.perc) * Math.cos(angleSlice * i - Math.PI / 2) + center.x
    )
    .attr(
      "cy",
      (d, i) =>
        rScale(d.perc) * Math.sin(angleSlice * i - Math.PI / 2) + center.y
    )
...
function draw_event_heatmap() {
  ...
  for (let gw of gws) {
    for (let stat of stat_names) {
      let d =
        gw in raw_data ? (stat in raw_data[gw] ? raw_data[gw][stat] : []) : [];
  ...
  for (let stat of stat_names) {
    let s = getSum(
      (type_data[stat] || []).map((i) => parseInt(i.points * i.multiplier))
    );
  ...
  let valmax = Math.max(
    Math.max(
      ...data.filter((i) => i[1] != "Total" && i[0] != "Total").map((i) => i[2])
    ),
    1
  );
  let valmin = Math.min(
  ...
  let stmax = Math.max(
    Math.max(
      ...data.filter((i) => i[0] == "Total" && i[1] != "Total").map((i) => i[2])
    ),
    1
  );
  let stmin = Math.min(
  ...
  var sColor = (d) => {
    let p = d3
      .scaleLinear()
      .domain([0, -valmin / (valmax - valmin), 1, 100])
      .range(["#e19797", "#ffffff", "#7FD3D9", "#7FD3D9"]);
  ...
  var gwColor = (d) => {
    let p = d3
      .scaleLinear()
      .domain([0, 1, 100])
      .range(["#ffffff", "#7FD3D9", "#7FD3D9"]);
    return p((d - gwmin) / (gwmax - gwmin));
  };
  var totColor = (d) => {
    let p = d3
      .scaleLinear()
      .domain([0, -stmin / (stmax - stmin), 1, 100])
      .range(["#e19797", "#ffffff", "#7FD3D9", "#7FD3D9"]);
...
function draw_gain_loss_candlestick() {
  ...
  const raw_svg = d3
    .select("#benefit-candlestick-chart")
    .insert("svg", ":first-child")
  ...
  let chart_body = svg.append("g").attr("clip-path", "url(#clip)");

  let xvals = _.range(1, 39);
  let x = d3.scaleBand().range([0, width]).domain(xvals).padding(0);
  svg
    .append("g")
    .attr("transform", "translate(0," + height + ")")
    .attr("class", "x-axis")
  ...
  chart_body
    ...
    .attr("y", y(max_y))
    .attr("class", (d) =>
      d % 2 == 0 ? "gw-bg gw-bg-even" : "gw-bg gw-bg-odd"
    );

  // zero line
  let zero_line = chart_body
    .append("g")
    .append("line")
    .attr("x1", x(0))
    .attr("y1", y(0))
  ...
  let candles = chart_body
    ...
    .attr("class", (d) =>
      d.part == 1 ? "candle candle-gain" : "candle candle-loss"
    )
    .attr("opacity", 0.9)
  ...
  function zoomin(event) {
    ...
    svg
      .selectAll("rect.gw-bg")
      .attr("x", (d) => x(d))
      .attr("width", x.bandwidth());
    svg.selectAll(".x-axis").call((g) =>
  ...
  function hoverBar(event, d) {
    ...
    let tg =
      '<span class="gain-blue">' +
      d.top_gains
        .map(
          (i) => app.fpl_element[i.id].web_name + " (" + rounded(i.net, 2) + ")"
        )
        .join("<br/>") +
      "</span>";
    let tl =
    ...
    if (d.part == 1) {
      document
        .getElementById("hover_gw_gain")
        .parentElement.classList.add("highlighted-div");
      document
        .getElementById("hover_top_gains")
        .parentElement.classList.add("highlighted-div");
    } else if (d.part == 2) {
      document
        .getElementById("hover_gw_loss")
        .parentElement.classList.add("highlighted-div");
      document
        .getElementById("hover_top_losses")
        .parentElement.classList.add("highlighted-div");
    }
  }
  ...
  function clearHover(event, d) {
    ...
    document
      .querySelectorAll(".highlighted-div")
      .forEach((e) => e.classList.remove("highlighted-div"));
  }
  ...
  function set_new_yaxis() {
    ...
    if (cumulative == true) {
      // default
      if (net_ch == false) {
        new_yvalues = filtered
          .map((i) => i.was)
          .concat(filtered.map((i) => i.current));
    ...
    let yt = candles.transition().duration(400);
    ...
    svg
      ...
      .transition()
      .duration(400)
      .call((g) => g.call(d3.axisRight(y).tickSize(width)))
      .call((g) =>
        g
          .selectAll(".tick line")
          .attr("stroke-opacity", 0.2)
    ...
    zero_line.transition().duration(400).attr("y1", y(0)).attr("y2", y(0));
  ...
  function set_x_by_net(yt) {
    let net_ch = document.getElementById("net_switch").checked;
    if (net_ch == true) {
      yt.attr("x", (d) => x(d.gw) + x.bandwidth() * perc_gap).attr(
        "width",
        x.bandwidth() * (1 - 2 * perc_gap)
      );
    } else {
      yt.attr("x", (d) =>
        d.part == 1
          ? x(d.gw) + x.bandwidth() * perc_gap
          : x(d.gw) + x.bandwidth() * (0.5 + perc_gap)
...
function draw_risk_reward_plot() {
  ...
  let chart_body = svg.append("g").attr("clip-path", "url(#clip2)");

  // x axis
  let max_x = 100;
  let min_x = 0;
  // let xvals = data.map(i => i.risk)
  let x = d3.scaleLinear().range([0, width]).domain([min_x, max_x]);
  let x_group = svg
    .append("g")
    // .attr('transform', 'translate(0,' + height + ')')
    .attr("class", "x-axis");
  ...
  let y = d3.scaleLinear().domain([min_y, max_y]).range([height, 0]);
  let y_group = svg.append("g").attr("class", "y-axis");
  ...
  function setup_axes(x_in, y_in) {
    ...
    y_group
      .call(d3.axisRight(y_in).tickSize(width))
      .call((g) =>
        g.selectAll(".tick text").attr("x", -5).attr("text-anchor", "end")
      );
    ...
    svg.call((s) =>
      s.selectAll(".tick").attr("font-size", "6pt").attr("fill", "white")
    );
    svg.call((s) => s.selectAll(".tick text").attr("fill", "white"));
  }
  ...
  function hoverRiskBar(event, d) {
    ...
    document.getElementById("risk_hover_diff_rate").innerHTML =
      rounded(d.risk) + "%";
    ...
    let tg =
      ...
      d.top_bets_for
        .map(
          (i) =>
            app.fpl_element[i.id].web_name +
            " (" +
            rounded(i.own_rate, 2) +
            "%)"
        )
        .join("<br/>") +
      "</span>";
    let tl =
      ...
      d.top_bets_against
        .map(
          (i) => app.fpl_element[i.id].web_name + " (" + rounded(i.eo, 2) + "%)"
        )
        .join("<br/>") +
      "</span>";
...
function draw_tree_map() {
  ...
  if (_.isEmpty(player_data)) {
    return;
  }

  let gain = app.user_ownership_gain_loss.gains;
  let player_gains = Object.fromEntries(
    _(gain)
      .groupBy("id")
      .map((i, v) => [v, getSum(i.map((j) => j.net))])
      .value()
  );
  ...
  //     .domain([0, 1])
  //     .range([.5, 1])
  var fontsize = d3.scaleLinear().domain([0, 1]).range([2, 6]);
  ...
  refresh_point_dist = () => {
    ...
    try {
      gain_switch =
        document.getElementById("actual_gain_switch")?.checked || false;
    } catch {
      gain_switch = false;
    }
    ...
    if (gain_switch) {
      player_data.forEach((e) => {
        e.value = Math.max(0, e.gain);
      });
      group_max = Object.fromEntries(
        _(player_data)
          .groupBy("pos")
          .map((i, v) => [v, Math.max(...i.map((j) => j.gain))])
          .value()
      );
      player_data.forEach((e) => {
        e.ratio = e.gain / group_max[e.pos];
      });
      ...
      group_max = Object.fromEntries(
        _(player_data)
          .groupBy("pos")
          .map((i, v) => [v, Math.max(...i.map((j) => j.points_total))])
          .value()
      );
      player_data.forEach((e) => {
        e.ratio = e.points_total / group_max[e.pos];
      });
      sorted_data = _(player_data).orderBy(["points_total"], ["desc"]).value();
      text_func = (d) => `${d.data.name} (${rounded(d.data.value)})`;
      pos_text = (d) => `${d.data.name} (${rounded(d.value)})`;
    ...
    let pos_data = _(sorted_data)
      .groupBy("pos")
      .map((i, v) => ({ name: v, children: i }))
      .value();
    ...
    d3
      ...
      .paddingTop(10)
      .paddingRight(3)
      .paddingInner(2)(newroot);

    let newleaves = svg.selectAll(".tm-leaves").data(newroot.leaves());
    ...
    newleaves
      ...
      .style("fill", function (d) {
        return color(d.parent.data.name, d.data.ratio);
      });

    let newtext = svg.selectAll(".tm-text").data(newroot.leaves());
    ...
    svg
      .selectAll(".tm-text")
      .text((d) => text_func(d))
      .attr("font-size", (d) => fontsize(d.data.ratio) + "pt");

    let newpostext = svg
      .selectAll(".pos-summary")
...
function draw_tree_map_loss() {
  ...
  plot_now = () => {
    group_max = Object.fromEntries(
      _(player_data)
        .groupBy("pos")
        .map((i, v) => [v, Math.max(...i.map((j) => j.loss))])
        .value()
    );
    player_data.forEach((e) => {
      e.ratio = e.loss / group_max[e.pos];
    });
...
function draw_predicted_realized_diff() {
  ...
  const raw_svg = d3
    .select("#season-xp-real-diff-plot")
    .append("svg")
    .attr(
      "viewBox",
      `0 0  ${width + margin.left + margin.right} ${
  ...
  let val_sum = [
    0,
    ...values.map((i) => i.total_exp_diff),
    ...values.map((i) => i.total_real_diff),
  ];
  ...
  let markers = _.range(
    Math.min(100 * Math.ceil(min_y / 100), 0),
    max_y,
    100
  ).filter((i) => i != 0);
  ...
  titles
    ...
    .style("font-size", "6px")
    .html(
      `<img src="static/images/logo_128.png" width="12"><span style="margin-left: 2px;">FPL Optimized</span>`
    );
  ...
  let chips = _.toPairs(app.team_data)
    .map((i) => [i[0], i[1].active_chip])
    .filter((i) => i[1] != null);
...
function draw_team_season_visual() {
  ...
  svg
    ...
    .call(d3.axisTop(x).tickSize(0))
    .call((g) =>
      g.selectAll(".tick text").attr("opacity", 0).style("display", "none")
    );
  ...
  if (app.show_info_ts) {
    ...
    svg.call((g) =>
      g.selectAll(".x-axis-2").style("transform", "translate(0px, -3px)")
    );
  }
  ...
  let pos_grp = body.append("g").selectAll().data(pos_data).enter();
  ...
  let markers = body.append("g").selectAll().data(actual_markers).enter();
  ...
  let gw_markers = body.append("g").selectAll().data(data).enter();
  ...
  let text_group = body.append("g").selectAll().data(actual_markers).enter();
  ...
  text_group
    ...
    .attr(
      ...
      (d) =>
        x(d.sort_order) +
        (app.show_pts_on_ts ? (x.bandwidth() * 5) / 12 : x.bandwidth() / 2)
    )
    .attr(
      "y",
      (d) => (y(d.sort_finish) + y(d.sort_start)) / 2 + y.bandwidth() / 2 + 1
    )
    .attr("font-size", "4pt")
    // .attr("fill", "black")
    // .attr("fill", "#ffc100")
  ...
  if (app.show_info_ts) {
    ...
    let gw_pts_text = (e) => {
      if (
        e.entry_history &&
        e.entry_history.event_transfers_cost &&
        e.entry_history.event_transfers_cost > 0
      ) {
        return (
          e.entry_history.points +
          " (-" +
          e.entry_history.event_transfers_cost +
          ")"
        );
    ...
    let rank_vals = Object.entries(td).map((i) => [
      i[0],
      _.get(i[1], "entry_history.overall_rank"),
    ]);
...
function draw_point_origin_graph() {
  ...
  if (true) {
    ...
    let y = d3
      .scaleBand()
      .domain(_.range(row_count))
      .range([0, x.step() * row_count])
      .paddingInner(0.15);
    ...
    let box_holders = group_holder
      .selectAll()
      .data((d, g) =>
        d.col_boxes.map((val, inner_col) => {
          return { val: val, inner_col: inner_col, group: g };
        })
      )
      .enter();
    ...
    let single_node = box_holders
      .selectAll()
      .data((d) =>
        d.val.map((k, r) => {
          return { val: k, row: r, col: d.inner_col, group: d.group };
        })
      )
      .enter();
    ...
    let legend_box = s1
      ...
      .attr(
        "transform",
        `translate(${(width - cat_keys.length * single_width) / 2}, ${
          y_end + 30
        })`
      );

    let legend_entry = legend_box.selectAll().data(all_values).enter();

    let single_entry = legend_entry
      .append("g")
  ...
  if (true) {
    ...
    s2.append("text")
      ...
      .attr("fill", "white")
      .text(
        "Total Points, Counts, Points per Pick by Decision Age and Acquisition Type"
      );
    ...
    let x = d3
      ...
      .range([0, width])
      .domain(
        ["Age"].concat(group_names.map((i) => i[0])).concat(["Total", "Perc."])
      )
      .padding(0.05);
    graph_inner
      .append("g")
      ...
      .map((age) =>
        group_names.map((gr) => {
          return {
            ...
            ppp: _.round(
              (_.get(total, gr[1] + "." + age) || 0) /
                (_.get(count, gr[1] + "." + age) || 1),
              1
            ),
          };
        })
    ...
    let color_range = d3
      .scaleLinear()
      .range(["#ffffff", "#00fff8"])
      .domain([0, d3.max(values_as_list.map((i) => i.total))]);

    // y labels
    y_labels = graph_inner
      .append("g")
      .selectAll()
    ...
    let cell = graph_inner.append("g").selectAll().data(values_as_list).enter();
    ...
    cell
      ...
      .attr("fill", "white")
      .text((d) =>
        d.count != "" ? d.count + " picks / " + d.ppp + " ppp" : ""
      );

    let total_cells = graph_inner
      .append("g")
      .selectAll()
    ...
    total_cells
      ...
      .text(
        (d) =>
          (data.age_cnt[d[0]] || 0) +
          " picks / " +
          _.round(d[1] / (data.age_cnt[d[0]] || 1), 1) +
          " ppp"
      );
    ...
    let gr_sums = _(values_as_list)
      .groupBy("group_text")
      .map((val, key) => [
        key,
        _.sum(val.map((i) => parseInt(i.val))),
        data.grp_cnt[gr_dict[key]] || 0,
      ])
      .value();

    let total_groups = graph_inner
      .append("g")
      .selectAll()
      .data(gr_sums)
      .enter();
    ...
    total_groups
      ...
      .attr("fill", "white")
      .text(
        (d) => d[2] + " picks / " + _.round(d[1] / (d[2] || 1), 1) + " ppp"
      );
  ...
  if (true) {
    ...
    let x = d3.scaleBand().range([30, width]).domain(gws).padding(0.1);
    let y = d3
      .scaleBand()
      .range([0, 150])
      .domain(_.range(15))
    ...
    let gw_chips = _(data.data)
      .mapValues((val, key) => val.active_chip)
      .value();
...

    {
      ...
      let point_holder = s3_p1
        .append("g")
        .attr("transform", "translate(0, 10)");

      let leg_box = point_holder
        .selectAll()
      ...
      let type_vals = _(data.all_players)
        .groupBy("origin")
        .map((val, key) =>
          _(val)
            .groupBy("gw")
            .map((ival, ikey) => {
              return {
                cnt: ival.filter((i) => i.multiplier > 0).length,
                val: _.sum(ival.map((i) => i.eff_points)),
                gw: ikey,
                group: key,
              };
            })
  ...
  raw_svg.attr(
    "viewBox",
    `0 0  ${width + margin.left + margin.right} ${
      n_height + margin.top + margin.bottom
    }`
  );
}
...
async function get_points() {
  return read_cached_rp(app.season)
    .then((data) => {
...
async function get_538_data() {
  return $.ajax({
    ...
    success: (data) => {
      ...
      app.fdr_data = Object.freeze(
        final_data.filter((i) => i.league == "Barclays Premier League")
      );
    },
    error: (xhr, status, error) => {
      console.log(error);
...
async function fetch_main_data() {
  return read_cached_static(app.season)
    .then((data) => {
