...
var app = new Vue({
  ...
  methods: {
    ...
    updateColorsFromCSS() {
      ...
      this.colors = [
        themeColors.chartGoal,
        themeColors.chartAssist,
        themeColors.chartCs,
      ];
    },
    calculate() {
    ...
    calculate_single(idx) {
      ...
      // then
      let g_r = (this.goal_rate[idx] = this.percentage_to_lambda(
        player.goal / 100
      ));
      let a_r = (this.assist_rate[idx] = this.percentage_to_lambda(
        player.assist / 100
      ));
      let gc_r = (this.gc_rate[idx] = this.percentage_to_lambda(
        (100 - player.cs) / 100
      ));
      ...
      let gc_pts = (this.gc_points[idx] = gc_p
        .map((v, i) => point_rates["2gc"] * v * Math.floor(i / 2))
        .reduce((a, b) => a + b, 0));
      let csgc_pts = (this.csgc_points[idx] = cs_pts + gc_pts);

      let ep = (this.expected_points[idx] = goal_pts + assist_pts + csgc_pts);

      let goal_data = _.range(0, 11).map((g) => {
        return {
          count: g,
          points: point_rates["goal"] * g,
          probability: g_p[g] * 100,
        };
      });
      let assist_data = _.range(0, 11).map((g) => {
        return {
          count: g,
          points: point_rates["assist"] * g,
          probability: a_p[g] * 100,
        };
      });
      let cs_data = [
      ...
      let gc_data = _.range(0, 11).map((g) => {
        return {
          count: g,
          points: point_rates["2gc"] * Math.floor(g / 2),
          probability: gc_p[g] * 100,
        };
      });
          ...
          for (let gc of gc_data) {
            let entry = {
              ...
              points:
                ...
                a.points +
                gc.points +
                (gc.count == 0 ? cs_data[0].points : cs_data[1].points),
      ...
      all_combinations = _.filter(
        all_combinations,
        (o) => o.probability > 1e-4
      );
    ...
    plot_graphs() {
      ...
      let { goal_data, assist_data, cs_data, gc_data, ep, p_n } =
        this.graph_data[idx];

      if (this.first_draw) {
        this.graph_updates["goal"] = draw_generic({
          selector: "#goal_prob_graph",
          title: "Goal Probability vs Count",
          x_title: "Goals Scored",
          y_title: "Probability %",
          data: goal_data,
          color: this.colors[0],
        });
        this.graph_updates["assist"] = draw_generic({
          selector: "#assist_prob_graph",
          title: "Assist Probability vs Count",
          x_title: "Assists Made",
          y_title: "Probability %",
          data: assist_data,
          color: this.colors[1],
        });
        this.graph_updates["cs"] = draw_generic({
          selector: "#cs_prob_graph",
        ...
        this.graph_updates["gc"] = draw_generic({
          selector: "#gc_prob_graph",
          title: "Goals Conceded Probability vs Count",
          x_title: "Goals Conceded",
          y_title: "Probability %",
          data: gc_data,
          color: this.colors[2],
        });
      ...
      if (this.first_draw) {
        this.graph_updates["total"] = draw_ev_graph(
          all_combinations,
          ep_values,
          player_names,
          this.player_colors
    ...
    compare_two() {
      ...
      p1.forEach((c1) => {
        ...
        p1_better += c2
          .map((i) => ((i.probability / 100) * c1.probability) / 100)
          .reduce((a, b) => a + b, 0);
        // p2 better
    ...
    toggle_rate() {
      ...
      this.$nextTick(() => {
        let goal_values = this.players.map((i) =>
          this.percentage_to_lambda(i.goal / 100)
        );
        jQuery(".goal_rates").each((i, v) => {
          v.value = parseFloat(goal_values[i].toFixed(4));
        });
        let assist_values = this.players.map((i) =>
          this.percentage_to_lambda(i.assist / 100)
        );
        jQuery(".assist_rates").each((i, v) => {
          v.value = parseFloat(assist_values[i].toFixed(4));
        });
        let gc_values = this.players.map((i) =>
          this.percentage_to_lambda((100 - i.cs) / 100)
        );
...
function draw_ev_graph(data, avg_value, player_names, colors) {
  ...
  // Axis-y
  let y_high = Math.max(
    Math.max(...data.map((i) => i.map((j) => j.probability)).flat()) * 1.1,
    0.5
  );
  let y_low = 0;
  ...
  svg
    .call((g) => g.selectAll(".tick text").attr("fill", themeColors.svgText))
    .call((g) =>
      g
        .selectAll(".tick line")
  ...
  svg.call((g) => g.selectAll(".tick").style("font-size", font_size));
  ...
  let bars = holder.selectAll().data(flat_data);
  ...
  let avg_place = (e) =>
    x(Math.floor(e)) + x.step() * (e - Math.floor(e)) + x.bandwidth() / 2;
  let avg_line = together
    .append("line")
  ...
  let update_func = (new_data, new_avg, new_names) => {
    ...
    let new_x_high = Math.max(
      ...new_data.map((i) => i.map((j) => j.points)).flat()
    );
    let new_x_low = Math.min(
      ...new_data.map((i) => i.map((j) => j.points)).flat()
    );
    ...
    legend_box.attr("height", 7 * (new_names.length + 0.5));
    ...
    let new_y_high = Math.max(
      Math.max(...new_data.map((i) => i.map((j) => j.probability)).flat()) *
        1.1,
      0.5
    );
    ...
    new_avg_grp
      ...
      .attr("fill", (d, i) => colors(i))
      .text((d) => d.toFixed(2));
    let new_together = new_avg_grp
      .enter()
      .append("g")
      .attr("class", "avg_lines");
    new_together
      .append("line")
      .attr("x1", (d) => new_avg_place(d))
      .attr("x2", (d) => new_avg_place(d))
...
function draw_generic({ selector, title, x_title, y_title, data, color } = {}) {
  ...
  let update_func = (new_data, new_name) => {
    let new_y_high = Math.max(
      Math.max(...new_data.map((i) => i.probability)) * 1.1,
      0.5
    );
    y.domain([0, new_y_high]);
    yAxis
